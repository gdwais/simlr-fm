...
export function RatingPanel({
  ...
  initialCount: number;
  initialHistogram: Record<number, number>;
}) {
  ...
  const [hist, setHist] = useState<number[]>(
    Array.from({ length: 10 }, (_, i) => initialHistogram[i + 1] || 0)
  );
  ...
  useEffect(() => {
    ...
    void fetch(`/api/albums/${albumId}/upsert`, { method: "POST" });
  }, [albumId]);

  async function refreshStats() {
    const res = await fetch(`/api/albums/${albumId}/stats`);
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/ratings", {
        ...
        body: JSON.stringify({ albumId, score: value }),
      });
      const data = await res.json();
